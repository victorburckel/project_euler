...
size_t euler::problem13()
{
	...
	// max shift is 100 * 9 = 900
	const auto maxShift = static_cast< size_t >( std::floor( log10( numbers.size() * 9 ) ) );
	const auto digits = 10_u;
	std::vector< size_t > result( digits + maxShift );
	for( auto index : boost::irange( 0_u, digits + maxShift ) )
		result[ index ] = boost::accumulate( numbers | boost::adaptors::transformed( [ index ]( const auto& digits ) { return digits[ index ]; } ), 0_u );

	auto power = 1_u;
	const auto longer = boost::accumulate( result | boost::adaptors::reversed, 0_u, [ &power ]( size_t total, size_t current ) {
		total += current * power;
		power *= 10;
		return total;
	} );

	return boost::lexical_cast< size_t >( std::to_string( longer ).substr( 0_u, digits ) );
}
